...
use Neno\MarketingEssentials\Core\Content\ReservedIndividualCode\ReservedIndividualCodeCollection;
...
class PromotionCodeHelper {
    ...
    public function __construct(private readonly EntityRepository $individualCodesRepository, private readonly EntityRepository $reservedIndividualCodeRepository, private readonly PromotionCodeService $codeService)
    {
    }
    ...
    private function generateNewIndividualCodes(
        ...
        $codeEntries = $this->prepareCodeEntities($promotion->getId(), $newCodes);
        $codeEntries[0]["id"] = Uuid::randomHex();

        $this->individualCodesRepository->upsert($codeEntries, $context);
        $this->reserveIndividualCode($promotion->getId(), $codeEntries[0]["id"], $context);
            ...
            if ($individualCodesEntities === null) {
                ...
                if ($unusedCodeEntity === null) {
                    ...
                    $this->reserveIndividualCode(
                        $promotion->getId(), $unusedCodeEntity->getId(),
                        $context);
