...
namespace Dvsn\ProductFixedNet\Core\Content\Product\SalesChannel\Price;
...
class ProductPriceCalculator extends AbstractProductPriceCalculator implements ResetInterface
{
    ...
    {
        $configuration = $this->systemConfigService->get('DvsnProductFixedNet.config');

        if ($this->validateConfiguration($configuration, $context) === false) {
            $this->coreService->calculate($products, $context);
            return;
        }

        // since the default price calculation only has private methods we have to
        // completly overwrite the core service and copy/paste everything - just to
        // change one private method.
        // we also cant run the core service first and calculate afterwards
        // because we would have to copy/paste everything anyway plus we would
        // overwrite the previous calculations.
        $units = $this->getUnits($context);

        /** @var SalesChannelProductEntity $product */
        foreach ($products as $product) {
            // set current product for the internal tax calculation
            $this->product = $product;

            // continue with shopware default
    ...
    private function validateConfiguration(array $configuration, SalesChannelContext $salesChannelContext): bool
    {
        // plugin even active for this sales channel?
        ...
        // valid country
        if (!$salesChannelContext->getShippingLocation() instanceof ShippingLocation || !$salesChannelContext->getShippingLocation()->getCountry() instanceof CountryEntity || !isset($configuration['conditionCountry']) || !in_array($salesChannelContext->getShippingLocation()->getCountry()->getId(), (array) $configuration['conditionCountry'])) {
            // unknown or invalid country
            return false;
        }

        // check valid customer group
        if (!isset($configuration['conditionCustomerGroup']) || !in_array($salesChannelContext->getCurrentCustomerGroup()->getId(), (array) $configuration['conditionCustomerGroup'])) {
            // invalid
            return false;
        }

        // okay, lets fix the price
        return true;
    }

    /**
     * This is the only method we have to change.
     */
    private function getPriceForTaxState(Price $price, SalesChannelContext $context): float
    {
        if ($context->getTaxState() === CartPrice::TAX_STATE_GROSS) {
            ...
            // do we have a specific tax rule for the tax id of the product?
            if (!$tax instanceof TaxEntity || $tax->getRules()->count() < 1) {
                // return default
                return $price->getGross();
            }

            /** @var TaxRuleEntity $rule */
